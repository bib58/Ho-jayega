...
export default function Dashboard() {
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            setDashboardData(data.aagya);
            console.log("Dashboard Data:", data.aagya);
        }
        catch (err) {
                    ...
                    dashboardData.ratings.map((review, index) => (
                        <div key={index} className="flex max-sm:flex-col gap-5 sm:items-center justify-between py-6 border-b border-slate-200 text-sm text-slate-600 max-w-4xl">
                            ...
                            <div className="flex flex-col justify-between gap-6 sm:items-end">
                                ...
                                <button onClick={() => router.push(`/product/${review.product._id || review.product.id}`)} className="bg-slate-100 px-5 py-2 hover:bg-slate-200 rounded transition-all">View Product</button>
                            </div>
                        </div>
                    ))
                }
