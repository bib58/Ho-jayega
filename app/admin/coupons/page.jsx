...
export default function AdminCoupons() {
    ...
    const fetchCoupons = async () => {
        try {
            ...
            toast.error(error.response?.data?.message || "An error occurred while fetching coupons");
        }
    }
    ...
    const handleAddCoupon = async (e) => {
        ...
        try {
            ...
            const { data } = await axios.post('/api/admin/coupons', payload, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
            ...
            toast.error(error.response?.data?.message || "An error occurred while adding the coupon");
        }
    }
    ...
    const deleteCoupon = async (code) => {
        try {
            const token = await getToken();
            const confirmDelete = window.confirm(`Are you sure you want to delete the coupon with code "${code}"?`);
        ...
        <div className="text-slate-500 mb-40">
            <form onSubmit={(e) => toast.promise(handleAddCoupon(e), { loading: "Adding coupon..." })} className="max-w-sm text-sm">
                ...
                <label>
                    ...
                    <input type="date" placeholder="Coupon Expires At" className="w-full mt-1 p-2 border border-slate-200 outline-slate-400 rounded-md" name="expiresAt" value={formatDate(newCoupon.expiresAt)} onChange={handleChange}
                    />
                </label>
                        ...
                        <tbody className="divide-y divide-slate-200">
                            {coupons.map((coupon) => (
                                <tr key={coupon.code} className="hover:bg-slate-50">
                                    ...
                                    <td className="py-3 px-4 text-slate-800">{formatDate(coupon.expiresAt)}</td>
                                    ...
                                    <td className="py-3 px-4 text-slate-800">
                                        <Trash2 onClick={() => toast.promise(deleteCoupon(coupon.code), { loading: "Deleting coupon..." })} className="w-5 h-5 text-red-500 hover:text-red-800 cursor-pointer" />
                                    </td>
                                </tr>
                            ))}
                        </tbody>
