    ...
    return (
        <div className="flex-1 space-y-2 text-sm">
            <div className="flex items-start justify-between gap-4">
                <Image width={100} height={100} src={store.logo.url} alt={store.name} className="max-w-20 max-h-20 object-contain shadow rounded-full max-sm:mx-auto" />
                <Link href={`/shop/${store.username}`} target="_blank" className="flex items-center gap-2 text-blue-600 hover:text-blue-800 text-sm px-3 py-1 border border-blue-200 rounded-lg hover:bg-blue-50">
            ...
            <div className="flex items-center gap-2 text-sm ">
                <Image width={36} height={36} src={store.user.image_url} alt={store.user.name} className="w-9 h-9 rounded-full" />
                <div>
